...
function renderBoard() {
    ...
    jobOrders.forEach(jo => {
        ...
        // TAMPILKAN NAMA KLIEN DI KARTU
        let clientHtml = jo.clientName ? `<div style="font-size:10px; font-weight:bold; color:#456d91; margin-bottom:4px;">👤 ${jo.clientName}</div>` : '';
...
function renderSalaryTable(viewType, category) {
    ...
    Object.keys(report).sort().reverse().forEach(key => {
        ...
        data.items.forEach(i => { 
            ...
            let qtyDisplay = '-';
            if (i.type === 'Feed') {
                qtyDisplay = `<strong>${i.slides || 1}</strong> Slide`;
            } else if (i.type === 'Reels') {
                qtyDisplay = '1 Video';
            } else if (i.type === 'Story') {
                qtyDisplay = '1 Konten';
            } else if (i.type !== 'Adjust') {
                qtyDisplay = '1 Aset';
            }

            // SHOW CLIENT IN TABLE
            let clientTable = i.clientName ? `<br><span style="font-size:10px; font-weight:bold; color:#456d91;">👤 ${i.clientName}</span>` : '';
...
function preparePDFContent(periodKey, categoryType, callback) {
    ...
    items.forEach(i => {
        ...
        // --- Custom Nama Qty (Sama dengan tabel monitor) ---
        let qtyDisplay = '-';
        if (i.type === 'Feed') {
            qtyDisplay = `<strong>${i.slides || 1}</strong> Slide`;
        } else if (i.type === 'Reels') {
        ...
        <tr>
            ...
            <td style="padding:10px; border-bottom:1px solid #eee; font-size:11px;">
                ...
                <span style="font-size:9px; color:#777;">Client: ${i.clientName || '-'}</span>
            </td>
...
function saveManualJob() {
    ...
    const cl = document.getElementById('man-client').value; 

    if(!t) return alert('Judul harus diisi!');
    if(!cl) return alert('Nama Klien harus diisi agar masuk CRM!');
    
    const j = createJob(document.getElementById('man-type').value, t, 'MANUAL', 1, cl); 
    j.category = c; 
...
function generateWeeklyTarget() {
    ...
    const jobs = [];
    const clientName = "Weekly Routine";
    
    jobs.push(createJob('Feed', 'Feed Single (1S)', batchID, 1, clientName)); 
    jobs.push(createJob('Feed', 'Feed Carousel (2S)', batchID, 2, clientName));
    jobs.push(createJob('Feed', 'Feed Carousel (4S) A', batchID, 4, clientName)); 
    jobs.push(createJob('Feed', 'Feed Carousel (4S) B', batchID, 4, clientName));
    jobs.push(createJob('Reels', 'Weekly Reels #1', batchID, 1, clientName)); 
    jobs.push(createJob('Reels', 'Weekly Reels #2', batchID, 1, clientName));
    for(let i=1; i<=7; i++) jobs.push(createJob('Story', `Daily Story #${i}`, batchID, 1, clientName));
...
function showJobDetail(id) {
    ...
    document.getElementById('det-client-name').innerText = jo.clientName ? `👤 ${jo.clientName}` : '👤 Client: -';
...
// --- LOGIKA CUT-OFF PERIODE BARU ---
function getPeriodPath(jo) { 
    const d = jo.data.deadline ? new Date(jo.data.deadline) : new Date(); 
    const p = getPeriod(d); 
    return `my space/00 Freelance/${p.year}/${p.monthName}/${p.periodStr}/Master`; 
}
function getPeriod(dateObj) { 
    ...
    // Tgl 1-5: Mundur 1 bulan (P2)
    if (day >= 1 && day <= 5) {
        pName = 2;
        targetDate.setMonth(targetDate.getMonth() - 1);
    } 
    // Tgl 6-20: Bulan ini (P1)
    else if (day >= 6 && day <= 20) {
        pName = 1;
    } 
    // Tgl 21-Akhir: Bulan ini (P2)
    else if (day >= 21) {
        pName = 2;
    }

    const monthName = targetDate.toLocaleString('id-ID', { month: 'long' }); 
    const year = targetDate.getFullYear();

    return { 
        key: `${monthName} ${year} - P${pName}`, 
        monthName: monthName, 
...
// ==========================================
// 7. MASTER BACKUP & RESTORE SYSTEM
// ==========================================
    ...
    reader.onload = function(e) {
        try {
            ...
            if (content.tipe === "MASTER_BACKUP_TAUFIK") {
                if (confirm(`⚠️ PERINGATAN: Data akan DITIMPA dengan backup tanggal ${content.tanggal}.\nLanjutkan?`)) {
                    ...
                    alert("✅ Berhasil! Data Job & Client dipulihkan.");
                    location.reload(); 
                }
            } else if (Array.isArray(content)) {
                 if (confirm("Ini file backup versi LAMA (Hanya Job Order).\nRestore data ini?")) {
                    localStorage.setItem('jo_db_v47', JSON.stringify(content));
                    alert("✅ Berhasil Restore Job Order Lama.");
                    location.reload();
                 }
            } else {
